...
public class DashboardDataFetch extends CohortDataFetch {
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException, SQLException {
		...
		if (args.length > 2 && args[2].equals("metrics")) {
			...
			stmt = conn.prepareStatement(
					"select rid, schema from palantir.download_request where active and rid not in (select rid from palantir.download_request_file)");
		} else if (args.length > 2) {
			logger.info("processing " + args[2]);
			stmt = conn.prepareStatement("select schema from palantir.download_request where rid = ?");
			...
			stmt = conn.prepareStatement("select rid, schema from palantir.download_request where active");
		ResultSet rs = stmt.executeQuery();
		while (rs.next()) {
...
//				process(name, rid, false);
			...
			PreparedStatement stmt = conn.prepareStatement("update palantir.download_request_file set updated = now(), metadata = ?::jsonb where rid = ? and file = ?");
			...
			if (matched == 0) {
				stmt = conn.prepareStatement("insert into palantir.download_request_file values(?, ?, now(), ?::jsonb)");
	...
	static void metrics() throws SQLException {
		PreparedStatement stmt = conn.prepareStatement("select file from palantir.download_request_file where updated is not null and aggregate_column is not null");
		ResultSet rs = stmt.executeQuery();
		while (rs.next()) {
	...
	static void generateMetrics(String fileName) throws SQLException {
		PreparedStatement stmt = conn.prepareStatement("select ((metadata->>'modified')::jsonb->>'time')::timestamp as updated,aggregate_column,aggregate_type "
														+ "from palantir.download_request_file "
														+ "where file = ? "
														+ "  and (file,((metadata->>'modified')::jsonb->>'time')::timestamp) not in (select file,updated from palantir.download_request_file_metrics)");
		...
		while (rs.next()) {
			...
			case "text":
				updateStmt = conn.prepareStatement(""
							+ "insert into palantir.download_request_file_metrics "
							+ "select "
							+ "		?,"
							+ "		?::timestamp,"
							+ "		count(*) as rows,"
